...
/**
 * Checks if a value looks like a module type (file extension)
 * @param {unknown} value The value to check
 * @returns {boolean} True if the value looks like a module type
 */
function looksLikeModuleType(value) {
...
/**
 * Validates VirtualUrlPlugin configuration and returns warnings
 * @param {VirtualModules} modules The virtual modules configuration
 * @returns {string[]} Array of invalid property names that should trigger warnings
 */
function validateConfiguration(modules) {
	const warnings = [];

	for (const key of Object.keys(modules)) {
		if (key === "type") {
			const value = modules[key];
			if (looksLikeModuleType(value)) {
				warnings.push(key);
			}
		}
	}
...
class VirtualUrlPlugin {
	...
	constructor(modules, scheme) {
		...
		/** @type {string[]} */
		this._configurationWarnings = validateConfiguration(modules);
	}
	...
	apply(compiler) {
		...
		const configurationWarnings = this._configurationWarnings;
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				if (configurationWarnings.length > 0) {
					for (const property of configurationWarnings) {
						compilation.warnings.push(
							new WebpackError(
								`Invalid VirtualUrlPlugin configuration: The property "${property}" at the top level is likely a mistake. ` +
									'Example: `{ "myModule": { type: ".js", source: () => "..." } }` instead of `{ type: ".js", "myModule": ... }`.'
							)
						);
					}
				}
